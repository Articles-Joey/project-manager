...
export async function getCurrentPackageVersion() {
    try {
        ...
        // return currentPackage.am_audit_version;
        return currentPackage.audit_version;
    } catch (error) {
...
export async function performAudit(
    ...
    try {
        ...
        console.log("fix value:", fix);
        const auditCommand = fix ? 'npm audit fix --json' : 'npm audit --json';

        const { stdout } = await execPromise(auditCommand, { cwd: targetPath });
        output = stdout;
    ...
    try {
        ...
        // Write to project-manager-details.json inside am_project_manager
        const metadataPath = path.join(amDir, 'details.json');
